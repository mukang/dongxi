...
- (void)send:(DXClientRequest *)clientRequest progress:(void (^)(float))progressBlock completion:(void (^)(DXClientResponse *))completionBlock {
    ...
    switch ([clientRequest version]) {
        case DXClientRequestVersion1:
            urlString = [NSString stringWithFormat:DXCLIENT_REQUEST_URL_FORMAT, [clientRequest apiName]];
            break;
        case DXClientRequestVersion2:
            urlString = [NSString stringWithFormat:DXCLIENT_REQUEST_URL_FORMAT_V2, [clientRequest apiName]];
            break;
    ...
    NSURLSessionDataTask * task = [self.session dataTaskWithRequest:urlRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error) {
            ...
            if (serializeError) {
                ...
                if (clientRequest.version == DXClientRequestVersion2) {
                    NSInteger errorno = [[responseData objectForKey:@"errno"] integerValue];
                    NSString *errmsg = [responseData objectForKey:@"errmsg"];
                    DXClientRequestErrorCode errcode;
                    if (errorno) {
                        switch (errorno) {
                            case DXClientResponseResultTypeV2UserInvalid:
                            case DXClientResponseResultTypeV2SessionInvalid:
                                errcode = DXClientRequestErrorRequestInvalid;
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    [[NSNotificationCenter defaultCenter] postNotificationName:@"DXClientResponseUserNeedRelogin" object:nil];
                                });
                                break;
                            default:
                                errcode = DXClientRequestErrorServerInternalError;
                                break;
                        }
                        error = [DXClientRequestError errorWithCode:errcode andDescription:errmsg];
                    } else {
...
//    DXClientLog(@"Client Send Data: %@", requestParams);
    ...
    if (clientRequest.version == DXClientRequestVersion2) {
        for (NSString *field in requestParams.allKeys) {
            id value = [requestParams objectForKey:field];
            if ([value isKindOfClass:[NSString class]]) {
                NSString *realField = [NSString stringWithFormat:@"x-%@", field];
                [*urlRequest addValue:[requestParams objectForKey:field] forHTTPHeaderField:realField];
            }
        }
        [*urlRequest addValue:@"1076" forHTTPHeaderField:@"x-uid"];
        if ([[clientRequest httpMethod] isEqualToString:@"GET"]) {
            [self prepareURLRequest:urlRequest withRequestParams:requestParams];
        } else {
#warning TODO:待补充
        }
    } else {
...
- (void)prepareURLRequest:(NSMutableURLRequest **)urlRequest withRequestParams:(NSDictionary *)requestParams {
    NSURL *url = [*urlRequest URL];
    NSDictionary *params = [requestParams objectForKey:@"request"];
    ...
    for (NSString *key in params.allKeys) {
        ...
        [paramParts addObject:paramString];
    }
    NSString *urlString = [NSString stringWithFormat:@"%@?%@", [url absoluteString], [paramParts componentsJoinedByString:@"&"]];
